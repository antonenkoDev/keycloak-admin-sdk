...
export class UserRoleMappingsApi {
  ...
  async addRealmRoles(userId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to add realm roles: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async removeRealmRoles(userId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to remove realm roles: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getAvailableRealmRoleMappings(userId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(
        `/users/${userId}/role-mappings/realm/available`,
        'GET'
      );
    } catch (error) {
