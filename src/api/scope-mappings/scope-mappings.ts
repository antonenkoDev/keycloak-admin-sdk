...
import { EffectiveRoleMappingsQuery, MappingsRepresentation } from '../../types/role-mappings';
...
export abstract class BaseScopeMappingsApi {
  ...
  async getAll(): Promise<MappingsRepresentation> {
    try {
      ...
      throw new Error(
        `Failed to get all scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getRealmScopeMappings(): Promise<RoleRepresentation[]> {
    try {
      return this.sdk.request<RoleRepresentation[]>(
        `${this.resourcePath}/scope-mappings/realm`,
        'GET'
      );
    } catch (error) {
      console.error(`Error getting realm scope mappings for ${this.resourcePath}:`, error);
      throw new Error(
        `Failed to get realm scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async addRealmScopeMappings(roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to add realm scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async deleteRealmScopeMappings(roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to delete realm scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getAvailableRealmScopeMappings(): Promise<RoleRepresentation[]> {
    try {
      return this.sdk.request<RoleRepresentation[]>(
        `${this.resourcePath}/scope-mappings/realm/available`,
        'GET'
      );
    } catch (error) {
      console.error(
        `Error getting available realm scope mappings for ${this.resourcePath}:`,
        error
      );
      throw new Error(
        `Failed to get available realm scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns Promise resolving to an array of role representations
   */
  async getEffectiveRealmScopeMappings(
    query?: EffectiveRoleMappingsQuery
    ...
    try {
      ...
      console.error(
        `Error getting effective realm scope mappings for ${this.resourcePath}:`,
        error
      );
      throw new Error(
        `Failed to get effective realm scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getClientScopeMappings(clientId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(
        `${this.resourcePath}/scope-mappings/clients/${clientId}`,
        'GET'
      );
    } catch (error) {
      console.error(
        `Error getting client scope mappings for ${this.resourcePath} and client ${clientId}:`,
        error
      );
      throw new Error(
        `Failed to get client scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async addClientScopeMappings(clientId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Error adding client scope mappings for ${this.resourcePath} and client ${clientId}:`,
        error
      );
      throw new Error(
        `Failed to add client scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async deleteClientScopeMappings(clientId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      ...
      console.error(
        `Error deleting client scope mappings for ${this.resourcePath} and client ${clientId}:`,
        error
      );
      throw new Error(
        `Failed to delete client scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getAvailableClientScopeMappings(clientId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(
        `${this.resourcePath}/scope-mappings/clients/${clientId}/available`,
        'GET'
      );
    } catch (error) {
      console.error(
        `Error getting available client scope mappings for ${this.resourcePath} and client ${clientId}:`,
        error
      );
      throw new Error(
        `Failed to get available client scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns Promise resolving to an array of role representations
   */
  async getEffectiveClientScopeMappings(
    clientId: string,
    query?: EffectiveRoleMappingsQuery
  ): Promise<RoleRepresentation[]> {
    if (!clientId) {
    ...
    try {
      ...
      console.error(
        `Error getting effective client scope mappings for ${this.resourcePath} and client ${clientId}:`,
        error
      );
      throw new Error(
        `Failed to get effective client scope mappings: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
