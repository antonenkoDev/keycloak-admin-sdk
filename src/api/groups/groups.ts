...
export class GroupsApi {
  ...
  async list(params?: GetGroupsParams): Promise<GroupRepresentation[]> {
    try {
      ...
      throw new Error(
        `Failed to retrieve groups list: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @throws {Error} If the request fails
   */
  async count(params?: GetGroupsCountParams): Promise<number> {
    try {
      const query = new URLSearchParams(params as any).toString();
      const endpoint = `/groups/count${query ? `?${query}` : ''}`;
      const response = await this.sdk.request<{ count: number }>(endpoint, 'GET');
      return response.count;
    } catch (error) {
      throw new Error(
        `Failed to count groups: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  async get(groupId: string): Promise<GroupRepresentationResponse> {
    if (!groupId) {
    ...
    try {
      ...
      return this.sdk.request<GroupRepresentationResponse>(endpoint, 'GET');
    } catch (error) {
      throw new Error(
        `Failed to get group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async update(groupId: string, group: GroupRepresentation): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to update group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getChildren(
    ...
    try {
      ...
      throw new Error(
        `Failed to get children for group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async createChild(groupId: string, child: GroupRepresentation): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to create child group for parent ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getMembers(groupId: string, params?: GetGroupMembersParams): Promise<UserRepresentation[]> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get members for group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async getManagementPermissions(groupId: string): Promise<ManagementPermissionReference> {
    ...
    try {
      ...
      throw new Error(
        `Failed to get management permissions for group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  async setManagementPermissions(
    ...
    try {
      ...
      throw new Error(
        `Failed to set management permissions for group ${groupId}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
