...
export class RolesByIdApi {
   ...
   */
  async getComposites(roleId: string, query?: GetRoleCompositesQuery): Promise<RoleRepresentation[]> {
    if (!roleId) {
      throw new Error('Role ID is required');
    }
  ...
  async getRealmRoleComposites(roleId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(`/roles-by-id/${roleId}/composites/realm`, 'GET');
    } catch (error) {
   ...
   * 
   * Endpoint: GET /{realm}/roles-by-id/{role-id}/composites/clients/{clientUuid}
   * 
   * @param roleId - ID of the role
   * @param clientUuid - UUID of the client
   * @returns Promise resolving to an array of role representations
   */
  async getClientRoleComposites(roleId: string, clientUuid: string): Promise<RoleRepresentation[]> {
    if (!roleId) {
      throw new Error('Role ID is required');
    }
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(`/roles-by-id/${roleId}/composites/clients/${clientUuid}`, 'GET');
    } catch (error) {
      console.error(`Error getting client role composites for role ID ${roleId} and client ${clientUuid}:`, error);
   ...
   */
  async updatePermissions(roleId: string, permissions: ManagementPermissionReference): Promise<ManagementPermissionReference> {
    if (!roleId) {
      throw new Error('Role ID is required');
    }
