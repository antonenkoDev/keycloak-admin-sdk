...
export abstract class BaseRoleMappingsApi {
  ...
  async getRealmRoleMappings(): Promise<RoleRepresentation[]> {
    try {
      return this.sdk.request<RoleRepresentation[]>(`${this.resourcePath}/role-mappings/realm`, 'GET');
    } catch (error) {
  ...
  async getAvailableRealmRoleMappings(): Promise<RoleRepresentation[]> {
    try {
      return this.sdk.request<RoleRepresentation[]>(`${this.resourcePath}/role-mappings/realm/available`, 'GET');
    } catch (error) {
   ...
   * @returns Promise resolving to an array of role representations
   */
  async getEffectiveRealmRoleMappings(briefRepresentation: boolean = true): Promise<RoleRepresentation[]> {
    try {
  ...
  async getClientRoleMappings(clientId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(`${this.resourcePath}/role-mappings/clients/${clientId}`, 'GET');
    } catch (error) {
  ...
  async addClientRoleMappings(clientId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      await this.sdk.request<void>(`${this.resourcePath}/role-mappings/clients/${clientId}`, 'POST', roles);
    } catch (error) {
  ...
  async deleteClientRoleMappings(clientId: string, roles: RoleRepresentation[]): Promise<void> {
    ...
    try {
      await this.sdk.request<void>(`${this.resourcePath}/role-mappings/clients/${clientId}`, 'DELETE', roles);
    } catch (error) {
  ...
  async getAvailableClientRoleMappings(clientId: string): Promise<RoleRepresentation[]> {
    ...
    try {
      return this.sdk.request<RoleRepresentation[]>(`${this.resourcePath}/role-mappings/clients/${clientId}/available`, 'GET');
    } catch (error) {
   ...
   * @returns Promise resolving to an array of role representations
   */
  async getEffectiveClientRoleMappings(clientId: string, briefRepresentation: boolean = true): Promise<RoleRepresentation[]> {
    if (!clientId) {
      throw new Error('Client ID is required');
    }
