...
import { cleanupTestEnvironment, setupTestEnvironment, TestContext } from '../utils/test-setup'; // Test timeout - using environment variable with fallback
    ...
    afterAll(async () => {
      try {
        ...
        if (
          ...
          clientRole &&
          clientRole.name &&
          clientRole.name.startsWith('test-client-role-')
        ) {
