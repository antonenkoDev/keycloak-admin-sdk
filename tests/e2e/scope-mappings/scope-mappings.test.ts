...
describe('Scope Mappings API E2E Tests', () => {
  beforeAll(async () => {
    ...
    console.log(`Created test client: ${testClient.clientId} (${clientId})`);
    ...
    console.log(`Created target client: ${targetClient.clientId} (${targetClientId})`);
    ...
    console.log(`Created test client scope: ${testClientScope.name} (${clientScopeId})`);
    ...
    console.log(`Created test role: ${testRole.name} (${roleId})`);
    ...
    console.log(`Created test client role: ${clientRole.name} (${clientRoleId})`);
  });
  ...
  describe('Client Scope Mappings', () => {
    ...
    test('should get available client scope mappings for a client', async () => {
      const clientScopeMappings = sdk.scopeMappings.forClient(clientId);
      const availableMappings = await clientScopeMappings.getAvailableClientScopeMappings(targetClientId);
    ...
    test('should get effective client scope mappings for a client', async () => {
      const clientScopeMappings = sdk.scopeMappings.forClient(clientId);
      const effectiveMappings = await clientScopeMappings.getEffectiveClientScopeMappings(targetClientId);
