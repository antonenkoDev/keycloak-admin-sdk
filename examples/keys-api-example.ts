...
import { KeyMetadataRepresentation, KeysMetadataRepresentation } from '../src/types/keys';
 ...
 */
async function getKeysMetadata(sdk: KeycloakClient): Promise<KeysMetadataRepresentation> {
  try {
...
function isActiveKey(
  key: KeyMetadataRepresentation,
  keysMetadata: KeysMetadataRepresentation
): boolean {
...
function getActiveSigningKey(
  keysMetadata: KeysMetadataRepresentation
): KeyMetadataRepresentation | null {
 ...
 */
function getKeysWithCertificates(
  keysMetadata: KeysMetadataRepresentation
...
async function example(): Promise<{
  ...
  try {
    ...
    throw new Error(
      `Error in keys example: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
